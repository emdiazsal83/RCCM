...
getCorr <- function(x, y, Ex, Ey){
  
  corPea <- cor(x, y, method="pearson")
  corSpe <- cor(x, y, method="spearman")
  
  return(c(corPea, corSpe))
...
pm <- proc.time()
corrs <- applyFuncStack2(rawV1, rawV2, E_V1, E_V2,  func="getCorr", mcCoresRow=5, mcCoresCol=5)
...
corrs_nm <- paste("ccm_cause_vals", V1, V2, min(years), max(years), block2, sep="_")
assign(corrs_nm, corrs)
...
fileName <- paste(paste(V1, "vs", V2, min(years), max(years), block2, "ccmCorrs", sep="_"), "RData", sep=".")
save(list=c(corrs_nm), file=paste(reposResults, fileName, sep=""))
